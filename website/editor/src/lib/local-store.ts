...
export function createLocalStore<S>(
  name: string,
  initialState: S,
  ...
  const $store: Store<S> & Partial<Local> = createStore(initialState, {
    name: `Local(${name})`,
  })
  ...
  stores.push($store as Store<S> & Local)
