...
const charAt = (str: string, index: number) => str.charAt(index)
for (let i = 0; i < keyStrUriSafe.length; i++) {
  baseReverseDic[charAt(keyStrUriSafe, i)] = i
}

export function compress(input?: string | null | void): string | void {
...
export function decompress(input?: null | void): void
export function decompress(input?: string | null | void): string | void | null {
  ...
  const updateData = () => {
    while (power !== maxpower) {
      ...
      if (dataPosition === 0) {
        dataPosition = 32
        dataVal = baseReverseDic[charAt(input as string, dataIndex++)]
      }
      bits |= (resb > 0 ? 1 : 0) * power
      power <<= 1
    }
  }
  const updateDataPower = (exp: number) => {
    bits = 0
    maxpower = 2 ** exp
    power = 1
    updateData()
