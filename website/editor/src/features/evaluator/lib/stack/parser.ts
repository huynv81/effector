...
/**
 * Turns an `Error`, or similar object, into a set of `StackFrame`s.
 */
export function parse(error: Error | string | string[]): StackFrame[] {
  if (error == null) {
...
function extractLocation(token: string): [string, number, number] {
  ...
  return [data[0] as string, data[1] as number, data[2] as number]
}
